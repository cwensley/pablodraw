...
namespace Pablo.Formats.Character
{
	public class Page
	{
		...
		class EmptyGenerator : IGenerateRegion
		{
			...
			public void TranslateColour(Point point, ref uint foreColour, ref uint backColour)
			{
			}
		}
		...
		public unsafe static void GenerateRegion(Canvas canvas, Bitmap bitmap, Rectangle rect, Size fontSize, BitFont font, Palette pal, bool iceColor, bool blinkOn, Point? cursorPosition, IGenerateRegion generator)
		{
			...
			uint* pCharStart;
			uint foreColor, backColor;
			uint cursorForeColor = 0, cursorBackColor = 0;
				...
				if (pt.Y >= 0 && pt.Y < canvas.Height)
				{
					for (pt.X = rDraw.Left; pt.X <= rDraw.InnerRight; pt.X++)
					{
						...
						if (pt.X >= 0 && pt.X < canvas.Width)
						{
							...
							{
								cursorForeColor = bd.TranslateArgbToData(~foreColor | 0xff000000);
								cursorBackColor = bd.TranslateArgbToData(~backColor | 0xff000000);
								cursor = true;
							}
